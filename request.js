...
const chooseUserAgent = (ua) => {
  console.log('ua: ', ua)
  if (ua && !userAgentList[ua]) return ua;
  const uas = userAgentList[ua] || userAgentList.mobile.concat(userAgentList.pc);
  return uas[(Math.random() * uas.length) >> 0];
}
const axios = Axios.create();
// axios.interceptors.request.use(config => (console.log('axios request:\n', JSON.stringify(config)), config));
// axios.interceptors.response.use(config => (console.log('axios response:\n', JSON.stringify(config)), config));
module.exports = async (method, url, data, options) => {
  ...
  if (typeof options.cookie === 'object') {
    headers['Cookie'] = Object.keys(options.cookie)
      .map(key => `${encodeURIComponent(key)}=${encodeURIComponent(options.cookie[key])}`).join('; ')
  ...
  if (options.crypto === 'weapi') {
    ...
    headers['Cookie'] = Object.keys(header)
      .map(key => `${encodeURIComponent(key)}=${encodeURIComponent(header[key])}`).join('; ');
    data.header = header
  ...
  if (options.proxy) {
    if (options.proxy.indexOf('pac') > -1) {
      ...
      const { hostname: host, port = 80 } = new URL(options.proxy)
      if (host) {
  ...
  try {
    ...
    try {
      answer.body = options.crypto === 'eapi' ? JSON.parse(encrypt.decrypt(body).toString()) : body
